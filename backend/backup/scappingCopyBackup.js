...
const get_specific_bike = async (link, input1, brand) => {
    ...
    let data_res_arr1 = await scrap_common_model(new_bike_url);

    if ('items' in data_res_arr1 || "upcomingCars" in data_res_arr1) {
        data_res_arr1.upcomingCars.items.map(item => {
            data_res_arr1.items.push(item)
        })
        for (const val of data_res_arr1.items) {
